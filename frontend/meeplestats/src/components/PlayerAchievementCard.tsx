...
import { Card, Box, Title, Skeleton, Paper, Badge, Image, Text } from "@mantine/core";
...
const PlayerAchievementCard = ({ username }: { username?: string | null }) => {
  ...
  return (
    <Card shadow="sm" radius="lg" className="!overflow-hidden !border !border-gray-100 !w-full !mb-6">
      ...
      <Box className="!p-4">
        {loading ? (
          ...
          <Text className="!text-gray-500 !text-center !p-4">No achievements found</Text>
        ) : (
          <Box className="!grid !grid-cols-1 md:!grid-cols-2 lg:!grid-cols-3 !gap-4">
            {achievements.map((achievement, index) => (
              <Paper
                ...
                shadow="xs"
                radius="md"
                className="!p-0 !overflow-hidden !border !border-gray-100 !transition-all hover:!shadow-md"
              >
                {/* Achievement Image */}
                <Box className="!relative !w-full !aspect-[2/1] !overflow-hidden !bg-gray-100">
                ...
                {/* Achievement Details */}
                <Box className="!p-4">
                  <Text fw={600} className="!text-gray-800 !mb-1">
                    {achievement.achievement_id || 'Achievement'}
                  </Text>

                  {achievement.description && (
                    <Text size="sm" c="dimmed" className="!text-gray-600 !mb-2 !line-clamp-2">
                      {achievement.description}
                    </Text>
                  )}
